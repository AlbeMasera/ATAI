...
class CrowdResponder:
    ...
    def get_batch_rating(self, batch_id: str) -> str:
        ...
        return str(d.get(batch_id, "Rating not available"))  # Return a rating or a default message
    ...
    def response(self, query: str, predicate: rdflib.term.URIRef) -> CrowdResponse:
        if not predicate:
            return CrowdResponse(self.graph, AnswerLabel.No, "Sorry, I couldn't find any predicates in your question. Should we try another question?")

        recognized_entity = self.embedding_recognizer.get_crowd_entity(query)
        if not recognized_entity:
            return CrowdResponse(self.graph, AnswerLabel.No, "Sorry, I couldn't find that entity in the dataset. Should we try another question?")
